...
**The full backend does not exist yet.** Three legacy read endpoints are active
today: `pendinglist` supplies the request queue and `patinfo` supplies the
patient header; `tariffdetails` supplies request tariff lines. The remaining operations use the prototype service shell.
...
| Direct Collection setup (all four services)                                                                                            | `Collection/Direct/Direct.jsx` → `DirectSetup`                                                                                                                                     | Refresh candidate CR/current patient detail and validate selected workflow. Local legacy mode permits pending-list tariff configuration; production uses backend eligibility. |
| Patient search / Existing Patients                                                                                                     | `Collection/Direct/Direct.jsx` → `PatientSearchPopover`                                                                                                                            | `searchPatientPage`; local mode deduplicates live pending CRs and enriches ten patient tiles per page. Phone search uses bounded detail batches/cache.                        |
...
| Tariff Details / building a bill, live tariff search, discounts                                                                        | `CollectionDetails/BillingDetails/BillingDetails.jsx`                                                                                                                              | Legacy `tariffdetails` loads raised lines; addable tariffs use `getTariffPage`. Blank Enter opens API 05/20 catalogue selection.                                              |
| IPD Advance / Package / Part Payment / Bill Settlement forms                                                                           | `CollectionDetails/BillingDetails/BillingDetails.jsx`                                                                                                                              | Account/settlement context comes from eligibility `workflowContext`; see API 19.                                                                                              |
| Per-line settlement audit popup ("Details" icon)                                                                                       | `BillingDetails.jsx` → `TariffDetailsDialog`                                                                                                                                       | Filters already-loaded flat lines for the chosen group; no additional API call.                                                                                               |
...
| `src/utilities/sessionService.js`, `src/services/{legacyHbimsPendingRequests,legacyHbimsPatientInfo}.js` | Live staged queue/patient reads; no dummy queue fallback. Legacy request eligibility is a temporary pass-through; request tariffs now load from `tariffdetails` (except advances). APIs 17/18 document the staged reads; API 19 documents current tile contracts and rendering.                                                                    | Keep session/backend constants in `sessionService.js`. Implement the existing request-detail and eligibility contracts without changing established UI validation/routing.                                                                                                                                                                                                                                                                                                                                                                 |
