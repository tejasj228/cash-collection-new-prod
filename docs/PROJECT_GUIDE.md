...
│   ├── features/cashCollection/  the entire feature — everything below is self-contained
│   │   ├── CashCollectionApplication.jsx   feature root: owns ALL cross-screen state (see §6)
│   │   ├── pages/
│   │   │   ├── DashboardPage.jsx      the live dashboard (KPIs, charts, treemap, hourly, recent tx)
│   │   │   └── ConfirmationPage.jsx   ⚠ DEAD CODE — see §10, nothing routes here anymore
│   │   ├── components/
│   │   │   ├── layout/Navigation.jsx        TopNav — Collection / Dashboard links + End/Start Shift
│   │   │   ├── home/HomeComponents.jsx      ModeTabs, RequestWorklist (the pending-requests table),
│   │   │   │                               DirectSelector (service tiles), EstimatesHome ⚠ dead, see §10
│   │   │   ├── patient/PatientComponents.jsx  PatientSearchPopover, DirectSetup, PatientBanner,
│   │   │   │                                 PatientPhotoLightbox, DirectCollectionIllustration
│   │   │   ├── workspace/
│   │   │   │   ├── CollectionWorkspace.jsx      the workspace shell: patient banner + form + payment
│   │   │   │   └── AccountWorkflowBuilder.jsx   form for account/settlement-style workflows
│   │   │   ├── charges/ChargeBuilder.jsx    form for tariff-entry workflows (search + add + discount)
│   │   │   ├── payment/PaymentCard.jsx      payment modes, POS terminal polling, manual fallback
│   │   │   ├── dialogs/Dialogs.jsx          TariffDetailsDialog, ConfirmDialog, ManualPaymentDialog
│   │   │   ├── dashboard/
│   │   │   │   ├── ShiftEndDialog.jsx       count-cash → confirm → (stale-shift variant) flow
│   │   │   │   ├── ShiftReport.jsx          the printable end-of-shift report
│   │   │   │   ├── DonutChart.jsx / MiniPie.jsx   chart primitives
│   │   │   └── print/PrintableBill.jsx      fixed-width monospaced receipt, revealed only by @media print
...
│   │   │   └── reportDates.js        date-mask/parse helpers + the real-clock shift-timing helpers
│   │   │                            (todayLocalIso, nowLocalIso, readShiftOpenedAt, writeShiftOpenedAt —
│   │   │                             see §10, these are prototype-only, not part of the API contract)
│   │   └── services/              the feature's own backend adapter (the REAL half, see §8)
...
│       ├── cashCollection.css        the big feature stylesheet (~6.5k lines)
│       └── reports.css               dashboard-page-only styles
...
`CashCollectionApplication.jsx` is a **big, deliberately centralized state
...
| -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| Pending Requests queue (search, filter panel, sort, 10-row pages)                                                                      | `components/home/HomeComponents.jsx` → `RequestWorklist`                                                                                                                           | `listPendingRequests(filters)`                                                                                                              |
| Direct Collection service tiles                                                                                                        | `HomeComponents.jsx` → `DirectSelector`                                                                                                                                            | none (services come from bootstrap)                                                                                                         |
| Direct Collection setup (CR lookup, existing-patient list for IPD, transaction type, billing service)                                  | `components/patient/PatientComponents.jsx` → `DirectSetup`                                                                                                                         | `checkEligibility(context)` before continuing                                                                                               |
| Patient search / existing-patient popover                                                                                              | `PatientComponents.jsx` → `PatientSearchPopover`                                                                                                                                   | `searchPatients(query)` (bootstrap's `patients` array is also filtered client-side today — see §10)                                         |
| Patient identity banner (ABHA, ward/bed, consultant, photo)                                                                            | `PatientComponents.jsx` → `PatientBanner`, `PatientPhotoLightbox`                                                                                                                  | none — comes from the `Patient` object already loaded                                                                                       |
| Tariff Details / building a bill, live tariff search, discounts                                                                        | `components/charges/ChargeBuilder.jsx`                                                                                                                                             | `getTariffs(filters)` for the searchable catalogue                                                                                          |
| IPD Advance / Package / Part Payment / Bill Settlement forms                                                                           | `components/workspace/AccountWorkflowBuilder.jsx`                                                                                                                                  | none directly — context comes from `checkEligibility`'s `workflowContext`                                                                   |
| Per-line settlement audit popup ("Details" icon)                                                                                       | `components/dialogs/Dialogs.jsx` → `TariffDetailsDialog`                                                                                                                           | none — renders `workflowContext.chargeBreakdown` already loaded                                                                             |
...
| Payment (Cash/Cheque)                                                                                                                  | `components/payment/PaymentCard.jsx`                                                                                                                                               | `postTransaction(command)`                                                                                                                  |
| Payment (Card/UPI) — POS terminal, auto-polled, manual fallback                                                                        | `PaymentCard.jsx` + `Dialogs.jsx` → `ManualPaymentDialog`                                                                                                                          | `initiateTerminalPayment`, `getTerminalPaymentStatus` (polled every 10s), then `postTransaction`                                            |
| Printed receipt/bill                                                                                                                   | `components/print/PrintableBill.jsx`                                                                                                                                               | none — renders the `printableData` already returned by `postTransaction`                                                                    |
| Refunds                                                                                                                                | Same components as collection, `requestType: "Refund"`                                                                                                                             | same as collection                                                                                                                          |
| Live Dashboard — KPI tiles, payment-mode/category donuts, OPD/IPD/Emergency billing-service treemap, hourly chart, recent transactions | `pages/DashboardPage.jsx`                                                                                                                                                          | **none today** — see §10, everything is aggregated client-side from the `recentTransactions` array already in memory                        |
| Recent Transactions → Reprint                                                                                                          | `DashboardPage.jsx`                                                                                                                                                                | **not wired to anything yet** — the button has no `onClick` (§10)                                                                           |
| Recent Transactions → Cancel bill                                                                                                      | `DashboardPage.jsx`                                                                                                                                                                | **client-only today** — `CashCollectionApplication.jsx`'s `cancelBill()` just patches local state; no backend call exists yet (§10)         |
| Export CSV                                                                                                                             | `DashboardPage.jsx`                                                                                                                                                                | none — builds the CSV from data already in memory                                                                                           |
| End Shift → count cash → confirm                                                                                                       | `components/dashboard/ShiftEndDialog.jsx`                                                                                                                                          | `prepareShiftClose()`, then `closeShift(command)`                                                                                           |
| Cross-day "you left a shift open overnight" detection                                                                                  | `ShiftEndDialog.jsx` + `CashCollectionApplication.jsx`                                                                                                                             | **prototype-only today**, via a real-wall-clock timestamp in `localStorage` — see §10 for how the real backend does this correctly for free |
| Start Shift again (same day or after a stale close)                                                                                    | `CashCollectionApplication.jsx`                                                                                                                                                    | `reopenShift(command)`                                                                                                                      |
| Reprint Receipt (top nav, visible only once shift closed)                                                                              | `components/layout/Navigation.jsx` → `TopNav`                                                                                                                                      | none — placeholder `window.print()` today                                                                                                   |
...
| ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `pages/ConfirmationPage.jsx` (`<Confirmation/>`, the "Collection Confirmed" success screen) | **Dead code.** Nothing ever sets `stage` to `"confirmation"`. The real flow prints and returns straight to the queue (`CashCollectionApplication.jsx`'s `confirm()` calls `resetHome()` directly).                                                                                                                                                 | Either delete the file, or wire it in if you want an explicit success screen — currently the UX is intentionally "print and move on."                                                                                                                                                                                                                                                                                                                                                                                                      |
| `components/home/HomeComponents.jsx`'s `EstimatesHome` + `stage: "estimates"`               | **Dead code / unreachable.** `startEstimate()` sets `mode: "direct"`, `stage: "home"` — it never sets `"estimates"`. Estimates are actually reached by setting Direct Collection's **Transaction Type** dropdown to "Estimation".                                                                                                                  | Delete `EstimatesHome` and the `stage === "estimates"` branch, or repurpose them as a shortcut into the same flow.                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `mocks/prototypeData.js`'s `collectionModes` fixture                                        | **Unused by the current dashboard.** `DashboardPage.jsx` derives its own payment-mode donut from `recentTransactions` directly.                                                                                                                                                                                                                    | Safe to ignore; don't spend backend effort computing this field unless you reintroduce a consumer.                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Dashboard aggregation (`getDashboard` / `getPendingRequestMetrics`)                         | **Defined in the contract, not called by the UI.** `DashboardPage.jsx` never calls a service method — it loads `recentTransactions` once (via bootstrap) and computes every KPI/chart/breakdown client-side in JavaScript.                                                                                                                         | Implement `getDashboard` anyway (it's in the 16-method contract and the OpenAPI spec) for future scale, but know that **today**, the thing the real backend must get right is returning a complete, correctly-tagged `recentTransactions` array — see [`docs/api/10`](./api/10-transactions-list.md) for exactly which fields the client-side aggregation reads (including `hospitalService`/`billingService`, added for the OPD/IPD/Emergency treemap, which is **not yet in `contracts/openapi.yaml`**'s `Transaction` schema — add it). |
| "Cancel bill" (Recent Transactions table)                                                   | **Client-only.** `CashCollectionApplication.jsx`'s `cancelBill(no)` just marks a row `Cancelled` in local React state (`txPatches`); nothing is sent to a server.                                                                                                                                                                                  | Add a real endpoint (proposed shape: [`docs/api/15-cancel-transaction-GAP.md`](./api/15-cancel-transaction-GAP.md)) and wire the button to call it before trusting this feature in production — right now a page reload silently undoes every "cancelled" bill.                                                                                                                                                                                                                                                                            |
...
| Cross-day "shift left open overnight" detection                                             | **Prototype-only workaround.** Because the mock's `businessDate` never advances (always `PROTOTYPE_DATA.todayIso`), `model/reportDates.js` invented `shiftOpenedAt`/`todayLocalIso`/`readShiftOpenedAt`/`writeShiftOpenedAt` — real wall-clock timestamps kept in `localStorage` — purely so the UI has _something_ to compare against for a demo. | **Delete this workaround once the real backend returns a real `businessDate`** in `prepareShiftClose()`'s response (§9.6) — the existing UI logic in `ShiftEndDialog.jsx` (`isStaleShift`) already compares `businessDate` correctly; the `localStorage` code only exists to fake that field's realism against the mock.                                                                                                                                                                                                                   |
...
| Ant Design (`antd`, `@ant-design/icons`)                                                    | Used only for `ConfigProvider`, `Alert`, `Button` (in the bootstrap loader/retry screen) and `Loader`. The rest of the app is hand-written CSS (`src/styles/*.css`), not Ant components.                                                                                                                                                           | Don't assume every control is an antd component — check the file before changing a form field.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `docs/` before this rewrite                                                                 | Described an app shape (`OverviewPage.jsx` + `ReportsPage.jsx` as separate dashboard/report screens, a `SideRail` left-nav, `src/services/cashCollectionApi.js`) that no longer matches this repo — those pages were consolidated into today's single `DashboardPage.jsx`, and the nav is `TopNav` (Collection / Dashboard).                       | Nothing to do — this document replaces those. If you find old references elsewhere (e.g. in commit messages), trust the current source over them.                                                                                                                                                                                                                                                                                                                                                                                          |
