...
| `shift_business_date`             | string         | `YYYY-MM-DD`. **The real calendar date this shift segment opened on** — not always "today". This is the field that drives the frontend's "you left a shift open overnight" messaging (`ShiftEndDialog.jsx`'s `isStaleShift`). Getting this right retires a `localStorage`-based workaround in the frontend — see `PROJECT_GUIDE.md` §10. |
