...
One file per endpoint. Every table, dropdown, dashboard chart, and popup in
...
| 03  | [request-detail.md](./03-request-detail.md)                   | `GET /requests/{requestId}`                              | Opening a queued request                                                      |
...
| 08  | [terminal-payments.md](./08-terminal-payments.md)             | `POST /terminal-payments`, `GET /terminal-payments/{id}` | Card/UPI POS flow                                                             |
...
| 13  | [shift-close.md](./13-shift-close.md)                         | `POST /shifts/{shiftId}/close`                           | Confirming "End shift"                                                        |
| 14  | [shift-reopen.md](./14-shift-reopen.md)                       | `POST /shifts/{shiftId}/reopen`                          | "Start Shift" / "Start today's shift"                                         |
| 15  | [cancel-transaction-GAP.md](./15-cancel-transaction-GAP.md)   | _proposed_ `POST /transactions/{no}/cancel`              | Recent Transactions "Cancel bill" (currently client-only)                     |
| 16  | [reprint-GAP.md](./16-reprint-GAP.md)                         | _proposed_ `GET /transactions/{no}/printable`            | Recent Transactions "Reprint" (currently unwired)                             |
...
`requestId` is a server correlation ID for support/debugging — it is not a
...
database's `character varying` columns: `"amount": "480.00"`, never
`"amount": 480`. `page`, `size`, and `total` in a paginated envelope are the
one exception — those are real JSON integers.

**Dates.** `req_date` / `date` fields use `DD/MM/YYYY`. Anything named
`*Iso` (`dateIso`, `todayIso`, `businessDate`) uses `YYYY-MM-DD`. Times use
...
**Opaque values.** Anything called `id`, `*Id`, `version`, `req_id`,
`requestId` (inside `data`, not the outer trace one), `shiftId`, or
`patientContextVersion` is a value the browser only ever echoes back — never
...
[`docs/PROJECT_GUIDE.md` §4](../PROJECT_GUIDE.md#4-environment-variables-and-runtime-configuration)).
