...
| 04  | [patient-search.md](./04-patient-search.md)                         | `GET /patients`                                                                                  | All four Direct patient pickers; local pending-list source                      |
...
| 19  | [Tariff tile contract](./19-request-tariff-details.md)              | Shared rules for existing request, eligibility, and bootstrap contracts; not a separate endpoint | API-driven OPD/IPD tariff/account/settlement rendering                          |
| 20  | [Direct pickers](./20-direct-pickers.md)                            | Existing patient search, tariff catalogue, and eligibility contracts                             | All four Direct service pickers, admission checks, and blank-Enter tariff popup |

## Conventions used in every file below

The active legacy payment-mode endpoint is documented separately in [21-legacy-payment-category-mapping.md](./21-legacy-payment-category-mapping.md). Its legacy envelope and field names differ from the target REST contracts below.
